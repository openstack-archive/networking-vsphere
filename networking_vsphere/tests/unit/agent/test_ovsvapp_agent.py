...
class TestOVSvAppL2Agent(base.TestCase):
    ...
    def setUp(self, get_port_ofport, set_secure_mode, mock_create_ovs_bridge,
              setup_base_flows, _init_ovs_flows, setup_security_br,
              setup_ovs_bridges, setup_integration_br, create_consumers,
              get_admin_context_without_session, OVSvAppPluginApi,
              PluginReportStateAPI, SecurityGroupServerRpcApi, RpcPluginApi,
              setup_logging, init):
        super(TestOVSvAppL2Agent, self).setUp()
        cfg.CONF.set_override('security_bridge_mapping',
                              "br-fake:fake_if", 'SECURITYGROUP')
        get_port_ofport.return_value = 5
    ...
    def test_notify_device_updated_host(self):
        ...
        with mock.patch.object(self.agent.ovsvapp_rpc,
            ...
            self.assertTrue(mock_update_port_binding.called)
            self.assertTrue(mock_get_device_details.called)
