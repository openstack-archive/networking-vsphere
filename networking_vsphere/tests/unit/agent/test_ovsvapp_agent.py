...
class TestOVSvAppAgentRestart(base.TestCase):
    ...
    def setUp(self, mock_ovs_init, mock_set_protocols, mock_get_port_ofport,
              mock_set_secure_mode, mock_create_ovs_bridge, mock_mod_ovs_flow,
              mock_tcp_udp_learning_flows, mock_setup_base_flows,
              mock_check_ovs_firewall_restart, mock_init_ovs_flows,
              mock_setup_security_br, mock_setup_ovs_bridges,
    ...
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge')
    def test_check_for_patch_ports_not_found(self, mock_ovs_bridge):
        self.agent.sec_br = mock.Mock()
        self.agent.int_br = mock.Mock()
        self.agent.tenant_network_types = 'vlan,vxlan'
        self.agent.bridge_mappings = {'fake_physnet0': 'br-eth3'}
    ...
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge')
    def test_check_ovsvapp_agent_restart(self, mock_ovs_bridge):
        self.agent.int_br = mock.Mock()
        self.agent.sec_br = mock.Mock()
        self.agent.tenant_network_types = 'vlan,vxlan'
        self.agent.bridge_mappings = {'fake_physnet0': 'br-eth3'}
...
class TestOVSvAppAgent(base.TestCase):
    ...
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge.set_secure_mode')
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge.get_port_ofport')
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge.get_port_name_list')
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge.get_ports_attributes')
    @mock.patch('neutron.agent.common.ovs_lib.OVSBridge.set_protocols')
    def setUp(self, mock_set_protocols, mock_get_port_attributes,
              mock_get_port_name_list,
    ...
    def test_setup_ovs_bridges_vxlan(self):
        ...
        with mock.patch.object(self.agent, 'setup_tunnel_br'
                ...
                mock.patch.object(self.agent, 'set_openflow_version'
                                  ) as mock_set_openflow, \
            ...
            self.assertTrue(mock_rarp_flow.called)
            self.assertTrue(mock_set_openflow.called)
    ...
    def test_notify_device_updated_vxlan_rpc_exception(self):
        ...
        vm_port1 = SamplePort(FAKE_PORT_1)
        vm = VM(FAKE_VM, [vm_port1])
        self.agent.ports_dict = {FAKE_PORT_1: vm_port1}
        self.agent.state = ovsvapp_const.AGENT_RUNNING
        self.agent.tenant_network_types = [p_const.TYPE_VXLAN]
        with mock.patch.object(self.agent.ovsvapp_rpc,
                               "update_device_binding",
