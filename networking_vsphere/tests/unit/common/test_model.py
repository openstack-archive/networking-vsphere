...
class CommonModelTestCase(base.TestCase):
    ...
    def test_model_entity(self):
        entity = model.ResourceEntity(key="key1",
                                      uuid="uuid1")
        self.assertTrue(entity.key == "key1", "Entity key does not match")
        self.assertTrue(entity.uuid == "uuid1", "Entity uuid does not match")
    ...
    def test_model_physicalnic(self):
        ...
        self.assertTrue(nic.key == key, "Phy nic key does not match")
        self.assertTrue(nic.name == name, "Phy nic name does not match")
        self.assertTrue(nic.mac_address == mac_address,
                        "Phy nic mac_address does not match")
        self.assertTrue(nic.config == config,
    ...
    def test_model_virtualswitch(self):
        ...
        self.assertTrue(dvs.key == key, "DVS key does not match")
        self.assertTrue(dvs.name == name, "DVS name does not match")
    ...
    def test_model_network(self):
        ...
        self.assertTrue(network.key == key, "Network key does not match")
        self.assertTrue(network.name == name, "Network name does not match")
    ...
    def test_model_port(self):
        ...
        self.assertTrue(port.key == key, "Port key does not match")
        self.assertTrue(port.name == name, "Port name does not match")
    ...
    def test_model_virtualnic(self):
        ...
        self.assertTrue(virtual_nic.key == key, "vnic key does not match")
        ...
        self.assertTrue(virtual_nic.vm_id == vm_id,
    ...
    def test_model_virtualmachine(self):
        ...
        self.assertTrue(vm.uuid == vm_uuid, "vm uuid does not match")
        self.assertTrue(vm.key == key, "vm key does not match")
        self.assertTrue(vm.name == vm_name, "vm name does not match")
        self.assertTrue(len(vm.vnics) == 1, "vm vnics size does not match")
