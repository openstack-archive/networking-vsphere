...
class OVSvAppDBTestCase(testlib_api.SqlTestCase):
    ...
    def test_check_to_reclaim_local_vlan_with_no_deletes(self):
        net1_port1_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net1')
        ovsvapp_db.get_local_vlan(net1_port1_info)

        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port1_info, False)
        self.assertFalse(status)

    def test_check_to_reclaim_local_vlan_with_delete(self):
        net1_port1_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net1')
        ovsvapp_db.get_local_vlan(net1_port1_info)

        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port1_info)
    ...
    def test_check_to_reclaim_local_vlan_multiple_ports(self):
        ...
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port_info)
        self.assertFalse(status)
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port_info)
        self.assertFalse(status)
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port_info)
    ...
    def test_release_stale_allocations(self):
        net1_port_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net1')
        # Allocate network.
        ovsvapp_db.get_local_vlan(net1_port_info)

        net2_port_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net2')
        # Allocate network.
        ovsvapp_db.get_local_vlan(net2_port_info)
        # Setup for release.
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port_info)
        self.assertTrue(status)
        status = ovsvapp_db.check_to_reclaim_local_vlan(net2_port_info)
        self.assertTrue(status)
        ret_val = ovsvapp_db.release_stale_allocations()
        self.assertEqual(2, ret_val)

    def test_release_stale_allocations_exception_and_retry(self):
        net1_port_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net1')
        # Allocate network net1.
        ovsvapp_db.get_local_vlan(net1_port_info)

        net2_port_info = self._form_port_info_dict(
            'fake_vcenter', 'fake_cluster', 'net2')
        # Allocate network net2.
        ovsvapp_db.get_local_vlan(net2_port_info)
        # Setup for release.
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port_info)
        self.assertTrue(status)
        status = ovsvapp_db.check_to_reclaim_local_vlan(net2_port_info)
        self.assertTrue(status)
        with mock.patch('neutron.db.api.get_session',
                        side_effect=Exception):
            ret_val = ovsvapp_db.release_stale_allocations()
            self.assertEqual(0, ret_val)
        # Next retry without exception.
        ret_val = ovsvapp_db.release_stale_allocations()
        self.assertEqual(2, ret_val)
    ...
    def test_get_stale_local_vlans_for_network(self):
        ...
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port1_info)
        self.assertTrue(status)
        status = ovsvapp_db.check_to_reclaim_local_vlan(net1_port2_info)
        self.assertTrue(status)
        ret_val = ovsvapp_db.get_stale_local_vlans_for_network('net1')
        self.assertEqual([('fake_vcenter', 'fake_cluster1'),
                          ('fake_vcenter', 'fake_cluster2')], ret_val)
