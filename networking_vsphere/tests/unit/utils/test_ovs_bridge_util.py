...
class TestOVSvAppIntegrationBridge(base.TestCase):
    ...
    def test_reclaim_local_vlan_vlan(self):
        with mock.patch.object(self.int_br, "delete_flows") as mock_delete_flow:
            ...
            self.assertEqual(mock_delete_flow.call_count, 2)
            mock_delete_flow.assert_any_call(dl_vlan = FAKE_SEG_ID,
...
class TestOVSvAppIntegrationBridge(base.TestCase):
    ...
    def test_reclaim_local_vlan(self):
        with mock.patch.object(self.tun_br, "delete_flows") as mock_delete_flow:
            self.tun_br.reclaim_local_vlan(FAKE_SEG_ID, FAKE_LVID)
            self.assertTrue(mock_delete_flow.called)
            self.assertEqual(mock_delete_flow.call_count, 2)
            mock_delete_flow.assert_any_call(dl_vlan=FAKE_LVID)
