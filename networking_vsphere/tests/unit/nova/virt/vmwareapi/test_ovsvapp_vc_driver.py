...
class OVSvAppVCDriverTestCase(test.TestCase):
    ...
    def test_wait_and_get_portgroup_details_None_networks(self):
        ...
        self.mox.StubOutWithMock(vutil, "get_object_property")
        vutil.get_object_property(mox.IgnoreArg(),
                                  vm_ref,
                                  "runtime.host").AndReturn(host)
        vutil.get_object_property(mox.IgnoreArg(),
                                  host,
                                  "network").AndReturn(networks)
        self.mox.ReplayAll()
        network_obj = self.conn._wait_and_get_portgroup_details(
    ...
    def test_wait_and_get_portgroup_details_with_network(self):
        ...
        vutil.get_object_property(mox.IgnoreArg(),
                                  vss,
                                  "config").AndReturn(mocked_props)
        vutil.get_object_property(mox.IgnoreArg(),
                                  mocked_props.distributedVirtualSwitch,
                                  "uuid").AndReturn(mocked_dvs_props)
        self.mox.ReplayAll()
        network_obj = (self.conn._wait_and_get_portgroup_details
                       (self.conn._session, vm_ref, vss.get('summary.name'),
                        self.instance))
    ...
    def test_wait_and_get_portgroup_details_with_exception(self):
