...
class L3HAVRRP(base.BaseAdminNetworkTest):
    ...
    @test.requires_ext(service='network', extension='hos')
    @test.idempotent_id('0d8c14fd-85e2-415a-869f-acfd91eab058')
    @test.services('network')
    def test_make_l3_agent_down(self):
        self.external_network_id = CONF.network.public_network_id
        name = data_utils.rand_name('router-')
        router = self.admin_client.create_router(
            name, external_gateway_info={
        ...
        def _connect_to_host_to_make_l3_agent_down(self, HOST):
            cmd = ('sudo service neutron-vpn-agent stop')
            subprocess.Popen(["ssh", "%s" % HOST, cmd],
                             shell=False,
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)

        def _connect_to_host_to_make_l3_agent_start(self, HOST):
            cmd = ('sudo service neutron-vpn-agent start')
            subprocess.Popen(["ssh", "%s" % HOST, cmd],
                             shell=False,
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)

        def _check_for_ip_address_of_qg_interface(self, HOST, router_id):
            cmd = ('sudo ip netns exec qrouter-' + router_id + ' ifconfig')
            ssh = subprocess.Popen(["ssh", "%s" % HOST, cmd],
                                   shell=False,
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
            output = ssh.stdout.readlines()
            variable = output[19].split()[0]
            return variable

        if self.value1 == active_host_name and count == 1:
            self._connect_to_host_to_make_l3_agent_down(HOST1)
            ip_address1 = self._check_for_ip_address_of_qg_interface(HOST1,
                                                                     router_id)
            ip_address2 = self._check_for_ip_address_of_qg_interface(HOST2,
                                                                     router_id)
            ip_address3 = self._check_for_ip_address_of_qg_interface(HOST3,
                                                                     router_id)
            if (ip_address1 != 'inet' and (ip_address2 == 'inet' or
                                           ip_address3 == 'inet')):
                self._connect_to_host_to_make_l3_agent_start(HOST1)
            else:
                msg = "L3 agent is up and router interface has a ip address"
                raise Exception(msg)

        if self.value2 == active_host_name and count == 1:
            self._connect_to_host_to_make_l3_agent_down(HOST2)
            ip_address1 = self._check_for_ip_address_of_qg_interface(HOST2,
                                                                     router_id)
            ip_address2 = self._check_for_ip_address_of_qg_interface(HOST1,
                                                                     router_id)
            ip_address3 = self._check_for_ip_address_of_qg_interface(HOST3,
                                                                     router_id)
            if (ip_address1 != 'inet' and (ip_address2 == 'inet' or
                                           ip_address3 == 'inet')):
                self._connect_to_host_to_make_l3_agent_start(HOST2)
            else:
                msg = "L3 agent is up and router interface has a ip address"
                raise Exception(msg)

        if self.value3 == active_host_name and count == 1:
            self._connect_to_host_to_make_l3_agent_down(HOST3)
            ip_address1 = self._check_for_ip_address_of_qg_interface(HOST3,
                                                                     router_id)
            ip_address2 = self._check_for_ip_address_of_qg_interface(HOST1,
                                                                     router_id)
            ip_address3 = self._check_for_ip_address_of_qg_interface(HOST2,
                                                                     router_id)
            if (ip_address1 != 'inet' and (ip_address2 == 'inet' or
                                           ip_address3 == 'inet')):
                self._connect_to_host_to_make_l3_agent_start(HOST3)
            else:
                msg = "L3 agent is up and router interface has a ip address"
                raise Exception(msg)
