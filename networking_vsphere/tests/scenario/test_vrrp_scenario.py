...
class L3HAVRRP(base.BaseAdminNetworkTest):
    ...
    def test_make_ha_link_down(self):
        ...
        def _connect_to_host_to_make_ha_link_down(self,
                                                  HOST,
                                                  ha_link,
                                                  router_id):
        ...
        def _connect_to_host_to_make_ha_link_up(self,
                                                HOST,
                                                ha_link,
                                                router_id):
            cmd1 = ('sudo ip netns exec qrouter-' + router_id)
        ...
        def _get_ha_link_from_namespace(self, HOST, router_id):
            cmd = ('sudo ip netns exec qrouter-' + router_id + ' ifconfig')
            ssh = subprocess.Popen(["ssh", "%s" % HOST, cmd],
                                   shell=False,
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
            output = ssh.stdout.readlines()
            get_ha_variable = output[0].split()[0].split("-")[0]
            get_ha_link = output[0].split()[0]
            return get_ha_variable, get_ha_link
