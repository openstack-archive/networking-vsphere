...
class OVSVAPPTestJSON(manager.ESXNetworksTestJSON):
    ...
    def test_network_port_and_allocated_count_in_cluster_vni_table(self):
        ...
        network_port_count = self._fetch_port_or_allocated_count_from_db(
            self.network['id'], "network_port_count")
        self.assertEqual(1, network_port_count)
        serverid2 = self._create_server_with_sec_group(
            name, net_id, group_create_body['security_group']['id'])
        self.assertTrue(self.verify_portgroup(self.network['id'], serverid2))
        allocated_count_after_second_server = \
            self._fetch_port_or_allocated_count_from_db(self.network['id'],
                                                        "allocated")
        self.assertNotEqual(allocated_count,
                            allocated_count_after_second_server)
        network_port_count_after_second_server = \
            self._fetch_port_or_allocated_count_from_db(self.network['id'],
                                                        "network_port_count")
        self.assertNotEqual(network_port_count,
                            network_port_count_after_second_server)
        self._delete_server(serverid1)
        self.assertTrue(self.verify_portgroup(self.network['id'], serverid2))
        allocated_count_after_server_delete = \
            self._fetch_port_or_allocated_count_from_db(self.network['id'],
                                                        "allocated")
        self.assertNotEqual(allocated_count_after_second_server,
                            allocated_count_after_server_delete)
        network_port_count_after_server_delete = \
            self._fetch_port_or_allocated_count_from_db(self.network['id'],
                                                        "network_port_count")
        self.assertNotEqual(network_port_count_after_second_server,
                            network_port_count_after_server_delete)
        self._delete_server(serverid2)
        del_status = self.verify_portgroup_after_vm_delete(self.network['id'])
        self.assertFalse(del_status)
        ...
        network_port_count_after_last_server_delete = \
            self._fetch_port_or_allocated_count_from_db(self.network['id'],
                                                        "network_port_count")
