...
class ESXNetworksTestJSON(base.BaseAdminNetworkTest,
    ...
    def _create_connection(self):
        ...
        try:
                ...
                LOG.info(msg)
                #connection = connect.SmartConnect(host=vcenter_ip, port=443, user=vcenter_username, pwd=vcenter_password, service="hostd", sslContext=context)
    ...
    def _verify_portgroup_after_vm_delete(self, network=None):
        net = self.admin_client.show_network(network)
#        net_segmentid = net['network']['provider:segmentation_id']
        net_segmentid = self._fetch_segment_id_from_db(net_id)
    ...
    def check_vapp_network_connectivity(self, ip_address, username,
                                          should_connect=True):
        try:
            linux_client = ssh.Client(ip_address, username,timeout=5,look_for_keys=True)
    ...
    def _fetch_segment_id_from_db(self, segmentid):
        cont_ip = "stack@cs-ccp-db-m1-clm"
        neutron_db = "'select lvid from ovsvapp_cluster_vni_allocations " \
                     "where network_id=\"" + segmentid + "\"';"
        cmd = ("sudo mysql -sN -D ovs_neutron -e" + ' '  + neutron_db)
        proc = subprocess.Popen(["ssh", "%s" % cont_ip, cmd],
                                shell=False,
                                stderr=subprocess.PIPE,
                                stdout=subprocess.PIPE)
    ...
    def _get_vapp_ip(self, host_n, vapp_name):
        ...
        for ip in ip_list:
            result = self.check_vapp_network_connectivity(ip,self.vapp_username)
            if result:
                return ip
                break
    ...
    def _create_multiple_server_on_different_host(self):
        ...
        while count < 5:
            ...
            if count is not 0:
                if str(serv_first_id['host_name']) == str(serv_second_id['host_name']):
                    if count == 4:
    ...
    def _create_multiple_server_on_same_host(self, network_id):
        ...
        while count < 5:
            ...
            if count is not 0:
                if str(serv_first_id['host_name']) != str(serv_second_id['host_name']):
                    if count == 4:
                        raise Exception('VM hosted on different host.')
                else:
                    return first_id, second_id
            count += 1
...
#        vm_obj = self.get_obj(content, [vim.VirtualMachine], vm)
    ...
    def _dump_flows_on_br_sec_to_verify_dhcp_ip(self, vapp_ipadd, dhcp_ip):
        HOST = self.vapp_username + "@" + vapp_ipadd
        time.sleep(self.build_interval)
        cmd = ('sudo ovs-ofctl dump-flows ' + self.br_inf + ' | grep table=0' + '| grep '+dhcp_ip)
