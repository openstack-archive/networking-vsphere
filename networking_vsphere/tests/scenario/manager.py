...
class ESXNetworksTestJSON(base.BaseAdminNetworkTest,
    ...
    def get_host_name(self, server_id):
        vcenter_ip = cfg.CONF.VCENTER.vcenter_ip
        vcenter_username = cfg.CONF.VCENTER.vcenter_username
        vcenter_password = cfg.CONF.VCENTER.vcenter_password
        content = self._create_connection(vcenter_ip, vcenter_username,
    ...
    def _create_multiple_server_on_different_host(self):
        ...
        while count < 3:
            ...
            server_1 = self.get_host_name(server_id)

            if count is not 0:
                if str(server_1['host_name']) == str(server['host_name']):
                    if count == 2:
                        return False, False
                else:
                    return str(server_1['vm_name']), str(server['vm_name'])
            server = server_1
            count += 1
