...
class ESXNetworksTestJSON(base.BaseAdminNetworkTest,
    ...
    def _verify_flows_when_vm_get_ip(self, vapp_ipadd, protocol, vlan,
        ...
        tenant_network_type = cfg.CONF.VCENTER.tenant_network_type
        if "vlan" == tenant_network_type:
                cmd = ('sudo ovs-ofctl dump-flows br-sec table=1' + ',' +
        ...
        if output[1:] == []:
                ...
                for output_list in output[1:]:
                    if "vlan" == tenant_network_type:
                        self.assertIn('dl_vlan=' + str(net_id), output_list)
                    else:
    ...
    def _verify_flows_after_vm_delete(self, vapp_ipadd, protocol, vlan,
        ...
        output = ssh.stdout.readlines()
        if output[1:] == []:
            LOG.info(_("VM Flows got deleted after vm delete as expected"))
        else:
            LOG.exception(_("Flows are present after VM delete: %s."),
                          output[1:])
