...
class OVSVAPPTestJSON(manager.ESXNetworksTestJSON):
    ...
    def test_mitigation_when_ovsvapp_vm_poweredoff(self):
        ...
        host_username = CONF.VCENTER.host_username
        ...
        ovs_id = find[0].split()[0]
        cmd1 = ('vim-cmd vmsvc/power.off' + ' ' + str(ovs_id))
        subprocess.Popen(["ssh", "%s" % HOST, cmd1],
                         shell=False,
                         stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE)

        time_to_sleep = CONF.VCENTER.agent_down_time
        ...
        self.assertEqual(host_mode[0], 'true')
        self._check_public_network_connectivity(floating_ip_toreach)

        self._status_of_host()

        devstack = CONF.VCENTER.devstack
        if devstack == 'yes':
            time.sleep(20)
