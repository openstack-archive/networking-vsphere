...
class MitigatedCluster(extension.NeutronClientExtension):
    resource = 'mitigated_cluster'
    resource_plural = 'mitigated_clusters'
    object_path = '/%s' % resource_plural
    resource_path = '/%s/%%s' % resource_plural
    versions = ['2.0']


class MitigatedClusterList(extension.ClientExtensionList, MitigatedCluster):
    """List all Mitigated clusters under Admin Context."""

    shell_command = 'vsphere-mitigated-cluster-list'
...
class MitigatedClusterShow(extension.ClientExtensionShow, MitigatedCluster):
    """Show mitigated info of a given cluster."""

    shell_command = 'vsphere-mitigated-cluster-show'
    allow_names = True

    def args2body(self, parsed_args):
        body = {'mitigated_cluster': {
    ...
    def get_data(self, parsed_args):
        ...
        try:
            ...
            not_found_message = (_("Unable to find %(resource)s of type "
                                   "'%(id)s'") %
...
def comman_args2body(parsed_args):
    ...
    body = {'mitigated_cluster': {
        'vcenter_id': parsed_args.vcenter_id,
        'cluster_id': parsed_args.cluster_id}}
    neutronV20.update_dict(parsed_args, body['mitigated_cluster'],
                           ['vcenter_id', 'cluster_id'])
    res_dict = body['mitigated_cluster']
...
class MitigatedClusterUpdate(extension.ClientExtensionUpdate,
                             MitigatedCluster):
    """Update a given mitigated cluster with given details."""

    shell_command = 'vsphere-mitigated-cluster-update'
    allow_names = True
    ...
    def run(self, parsed_args):
        ...
        print(_('Updated the given vcenter:cluster %(vc_id)s:%(clus_id)s') % {
              'vc_id': parsed_args.vcenter_id,
              'clus_id': parsed_args.cluster_id})
        return


class MitigatedClusterDelete(extension.ClientExtensionDelete,
                             MitigatedCluster):
    """Delete mitigation information of a specific cluster given."""

    shell_command = 'vsphere-mitigated-cluster-delete'
    ...
    def run(self, parsed_args):
        ...
        print(_('Deleted the given vcenter:cluster %(vc_id)s:%(clus_id)s') % {
              'vc_id': parsed_args.vcenter_id,
              'clus_id': parsed_args.cluster_id})
