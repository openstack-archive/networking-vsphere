...
class FaultCheckers(object):
    ...
    """Methods for fault checking of SOAP response.

    Per Method error handlers for which we desire error checking are defined.
    SOAP faults are embedded in the SOAP messages as properties and not as
    SOAP faults.
    """
    ...
    def retrieveproperties_fault_checker(resp_obj):
        ...
        missingSet. For example NotAuthenticated fault.
        """
        fault_list = []
        if not resp_obj:
            ...
            # the return value field has the specifics about the error, but
            # that's not the case with a timed out idle session. It is as bad
            # as a terminated session for we cannot use the session.
            # So setting fault to NotAuthenticated fault.
