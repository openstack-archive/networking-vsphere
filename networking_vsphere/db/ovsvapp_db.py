...
def _allocate_local_vlan(session, port_info, assign):
    ...
    try:
        ...
        try:
            ...
            if allocation:
                lvid = allocation.lvid
                count = allocation.update({'network_id': res['network_id'],
                                           'allocated': True,
...
def check_to_reclaim_local_vlan(port_info, decrement_count=True):
    ...
    with session.begin(subtransactions=True):
        ...
        try:
            ...
            count = 0
            if allocation.network_port_count >= 1:
                count = allocation.network_port_count - 1
                if decrement_count:
                    allocation.update({'network_port_count': count})
            if count == 0:
...
def release_local_vlan(port_info):
    session = db_api.get_session()
    with session.begin(subtransactions=True):
        res_keys = ['vcenter_id', 'cluster_id', 'network_id']
        res = dict((k, port_info[k]) for k in res_keys)
    ...
    with session.begin(subtransactions=True):
        try:
            ...
            for alloc in allocations:
                vcenter_clusters.append((alloc.vcenter_id, alloc.cluster_id))
        except sa_exc.NoResultFound:
