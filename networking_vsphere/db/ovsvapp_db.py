...
def _allocate_local_vlan(session, port_info, assign):
    lvid = None
...
def get_local_vlan(port_info, assign=True):
    ...
    with session.begin(subtransactions=True):
        try:
            if not assign:
                lvid = _allocate_local_vlan(session, port_info, assign)
                return lvid
            query = session.query(models.ClusterVNIAllocations)
...
def check_to_reclaim_local_vlan(port_info, decrement_count=True):
    count = -1
    session = db_api.get_session()
    with session.begin(subtransactions=True):
        res_keys = ['vcenter_id', 'cluster_id', 'network_id']
    ...
    try:
        ...
        with session.begin(subtransactions=True):
            ...
            allocations = (query.filter(
                models.ClusterVNIAllocations.pending_release == 1
            ).with_lockmode('update').all())
            LOG.debug('Stale allocations %s.', allocations)
        ...
        # Nothing to do, may be another controller cleared the record
        # We will just log and return.
        LOG.exception(_("Failed to delete stale allocations."))
        ...
        try:
            ...
            if allocations:
                ...
                for alloc in allocations:
                    alloc.update({'pending_release': True})
