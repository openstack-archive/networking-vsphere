...
class VCenterClusterDbMixin(vcenter_cluster.VCenterClusterPluginBase):
    ...
    def get_vcenter_cluster(self, context, vcenter_id, fields=None):
        ...
        LOG.info(_("Retreiving vCenter cluster information for vcenter_id:"
                   " %s"), vcenter_id)
        ...
        if len(query_objs) == 0:
            _msg = "No details found for venter: %s.", vcenter_id
    ...
    def update_vcenter_cluster(self, context, id, vcenter_cluster):
        ...
        LOG.info(_("Deleting the vcenter clusters %(clus_id)s with vcenter id"
                 "%(vcenter_id)s") % {'clus_id': clusters_list,
                 'vcenter_id': id})
