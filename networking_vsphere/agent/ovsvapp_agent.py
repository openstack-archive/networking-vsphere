...
class OVSvAppAgent(agent.Agent, ovs_agent.OVSNeutronAgent):
    ...
    def _process_uncached_devices_sublist(self, devices):
        ...
        try:
            ...
            devices = devices_ref
            self.do_ddl("AROON 1: " + str(ports))
            self.do_ddl("AROON 2: " + str(pending_ports))
            ...
            if len(ports) != len(pending_ports):
                ...
                for port in stale_ports:
                    self.do_ddl("AROON Refresh: " + str(port))
            ...
            # seconds of retry
            self.do_ddl("AROON 3: " + str(processed_ports))
            self.do_ddl("AROON 4: " + str(devices))
            if len(processed_ports) != len(devices):
                ...
                self.do_ddl("AROON 5: " + str(port_ids))
                self.do_ddl("AROON 6: " + str(devices))
                stale_ports = set(devices) - port_ids
                self.do_ddl("AROON Stale: " + str(stale_ports))
