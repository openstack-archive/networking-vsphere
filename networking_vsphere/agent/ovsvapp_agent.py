    ...
    def setup_security_br(self):
        ...
        if not self.phy_ofport:
            LOG.error(_("phy port not available on %s. Check your "
                        "security-bridge configuration, "
                        "Terminating the agent!"), secbr_name)
            sys.exit(1)
    ...
    def mitigate_ovs_restart(self):
        ...
        # done in ovs_neutron_agent
            ...
            try:
                ...
                ovsvapplock.release()
            LOG.info(_("done resetting the bridges post ovs restart"))
        except Exception:
            LOG.exception(_("exception encountered while mitigating the ovs"
                            " restart. will be retried in the next run"))
        ...
        if host == self.esx_hostname:
            ...
            while retry_count > 0:
                try:
                    ...
                    LOG.error(_("Retrying to create portgroup for network %s"),
                              network.name)
                    retry_count -= 1
                    if retry_count == 0:
