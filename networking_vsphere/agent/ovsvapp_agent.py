...
class OVSvAppAgent(agent.Agent, ovs_agent.OVSNeutronAgent):
    ...
    def __init__(self):
        ...
        # TODO(Adolfo) test to see if we can take this out
    ...
    def _populate_lvm(self, port):
        if port['network_id'] not in self.local_vlan_map.keys():
        ...
        lvm = self.local_vlan_map[port['network_id']]
        lvm.vif_ports[port['id']] = port
    ...
    def port_bound(self, port, net_uuid,
        ...
        if net_uuid not in self.local_vlan_map:
            ...
            # TODO(Adolfo). Should we return true or false?
            return True
    ...
    def _remove_from_untagged_devices(self, port):
        # TODO(Adolfo) we need to remove from list when removed from router
        if port.port_name in self.list_of_untagged_devices:
            del self.list_of_untagged_devices[port.port_name]

    def _remove_from_untagged_devices_by_vif_id(self, vif_id):
        names_of_ports_to_delete = []
        for port_name, port_info in self.list_of_untagged_devices.iteritems():
            if port_info['port'].vif_id == vif_id:
                names_of_ports_to_delete.append(port_name)
        for port_name in names_of_ports_to_delete:
            del self.list_of_untagged_devices[port_name]
    ...
    def process_deleted_ports(self, port_info):
        ...
        while self.deleted_ports:
            ...
            self._delete_and_unbind_vif_port(port)
        # Flush firewall rules after ports are put on dead VLAN to be
        # more secure
        self.sg_agent.remove_devices_filter(deleted_ports)

    def _delete_and_unbind_vif_port(self, port):
    ...
    def port_unbound(self, vif_id, net_uuid=None):
        ...
        if not self.local_vlan_map.get(net_uuid):
            LOG.info(_LI('port_unbound(): net_uuid %s not in local_vlan_map'),
                     net_uuid)
            # TODO(Adolfo) remove if not necessary
        ...
        # dvr_unbind_it
        # TODO(Adolfo) This needs to be fixed to make sure the port is VifPOrt
                    ...
                    for vnic in vm.vnics:
                        if self.tenant_network_type == p_const.TYPE_VLAN:
                            ...
                            port = {'mac_address': updated_port.mac_addr,
                                    'segmentation_id': updated_port.vlanid,
                                    'physical_network': updated_port.phys_net}
                            self._add_physical_bridge_flows(port)
            else:
                for vnic in vm.vnics:
                    if host_changed:
    ...
    def device_delete(self, context, **kwargs):
        ...
        try:
            ...
            if lvm is None:
                ...
                self._untagg_router_ports(network_id)
                self._remove_tunnel_flows_for_network(lvm)
        except Exception as e:
