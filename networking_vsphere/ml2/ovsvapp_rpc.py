...
class OVSvAppServerRpcCallback(object):
    ...
    def get_ports_details_list(self, rpc_context, **kwargs):
        ...
        agent_id = kwargs.get('agent_id')
        input_port_ids = kwargs.get('port_ids')
        vcenter_id = kwargs['vcenter']
        cluster_id = kwargs['cluster_id']
        LOG.debug("Port details requested by agent "
                  "%(agent_id)s for ports %(ports)s.",
                  {'ports': input_port_ids, 'agent_id': agent_id})
        out_ports = []
        for port_id in input_port_ids:
            ...
            LOG.debug(_("Adding port detail: %s."), entry)
...
class OVSvAppAgentNotifyAPI(object):
    ...
    def delete_vxlan_portgroup(self, context, network_info, host):
        cctxt = self.client.prepare(
            topic=self._get_device_topic(topics.DELETE))
        return cctxt.call(context, 'delete_vxlan_portgroup',
