...
class OVSvAppAgentDriver(object):
    ...
    def _notify_agent_and_update_db(self, network_info):
        ...
        if 'host' in network_info:
            ...
            if agent and 'host' in agent:
                ...
                LOG.error(_("Failed to find OVSvApp Agent with host "
                            ...
                            "for %(cluster) in vCenter %(vcenter)s."),
                          {'host': host,
        ...
        try:
            if status:
                success = self.notifier.delete_vxlan_portgroup(self.context,
                                                               network_info,
                                                               host)
            if success:
    ...
    def delete_port_postcommit(self, context):
        ...
        if self._is_device_owner_compute(context.current):
            ...
            if segment and segment[api.NETWORK_TYPE] == p_const.TYPE_VXLAN:
                ...
                try:
                    ...
                    if status:
                        ...
                        eventlet.spawn_n(self._notify_agent_and_update_db,
                                         net_info)
                except Exception:
